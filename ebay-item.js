...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Schritt 2: Artikeldaten abrufen — fieldgroups=PRODUCT liefert zusätzlich Produktdaten wie GTIN/EAN
    const itemRes = await fetch(
      `https://api.ebay.com/buy/browse/v1/item/get_item_by_legacy_id?legacy_item_id=${encodeURIComponent(itemId)}&fieldgroups=PRODUCT`,
      {
        headers: {
    ...
    // Hilfsfunktion: sucht in den Artikelmerkmalen (localizedAspects) nach einem Namen,
    // der einem der übergebenen Suchbegriffe ähnelt (z.B. "EAN", "Marke"/"Brand")
    function findAspect(patterns) {
      const aspects = itemData.localizedAspects;
      if (!Array.isArray(aspects)) return '';
      const hit = aspects.find(a => patterns.some(p => (a.name || '').toLowerCase().includes(p)));
      return hit?.value || '';
    }

    const ean = itemData.product?.gtins?.[0]?.identifierValue || itemData.product?.gtins?.[0] || findAspect(['ean', 'gtin', 'upc']) || '';
    const brand = itemData.product?.brand || findAspect(['marke', 'brand']) || '';

    // Bestand: eBay liefert bei Einzelstücken oft keine genaue Zahl (weil "1" selbstverständlich ist).
    // Falls keine Zahl geliefert wird, der Artikel aber grundsätzlich kaufbar ist, nehmen wir 1 an.
    const rawQty = itemData.estimatedAvailabilities?.[0]?.estimatedAvailableQuantity;
    const availabilityStatus = itemData.estimatedAvailabilities?.[0]?.estimatedAvailabilityStatus || '';
    let quantity = '';
    let quantityIsGuess = false;
    if (rawQty !== undefined && rawQty !== null) {
      quantity = rawQty;
    } else if (availabilityStatus !== 'OUT_OF_STOCK') {
      quantity = 1;
      quantityIsGuess = true;
    }
